...
export default function RoomFineModal({
  ...
  useEffect(() => {
    if (roomNumber) {
      setRoomDormers(dormers.filter((d) => d.room_number === roomNumber));
    } else {
      setRoomDormers([]);
    }
  }, [roomNumber, dormers]);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Apply Fine to Room</DialogTitle>
          <DialogDescription>
            Apply a shared fine to all residents in a specific room. Each
            resident will see the fine individually, but when any one resident
            pays it, the fine is automatically cleared for all others in the
            room.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="py-4 space-y-4">
            ...
            >
              ...
              <SelectContent>
                {availableRooms.map((room) => (
                  <SelectItem key={`room-${room}`} value={String(room)}>
                    Room {room}
                  </SelectItem>
                ))}
              </SelectContent>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm font-semibold text-blue-900 mb-2">
                    {roomDormers.length}{" "}
                    {roomDormers.length === 1 ? "Resident" : "Residents"} in
                    Room {roomNumber}
                  </p>
                  <div className="space-y-1">
                    {roomDormers.map((dormer) => (
                      <p key={dormer.id} className="text-xs text-blue-800">
                        • {dormer.first_name} {dormer.last_name} ({dormer.email})
                      </p>
                    ))}
                  </div>
                </div>
          ...
          <div className="space-y-2">
            <Label htmlFor="dateImposed">Date Imposed *</Label>
            <Input
              id="dateImposed"
              type="date"
              value={dateImposed}
          ...
          <div className="space-y-2">
            <Label htmlFor="reason">Fine Reason / Remarks *</Label>
            <Textarea
              id="reason"
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm text-yellow-800">
                  ...
                  <p className="text-xs">
                    ...
                    <strong>{roomDormers.length} linked fines</strong> of{" "}
                    <strong>₱{parseFloat(amount).toFixed(2)}</strong> each for
                    Room {roomNumber}.
                    <br />
                    When <strong>any one resident pays</strong> the fine, it
                    will be automatically cleared for all other residents in
                    the room.
                  </p>
                </div>
              </div>
        ...
        <DialogFooter>
          ...
          >
            {isSubmitting
              ? "Applying Fine..."
              : `Apply Fine to ${roomDormers.length || 0} ${
                  roomDormers.length === 1 ? "Resident" : "Residents"
                }`}
          </Button>
        </DialogFooter>
      </DialogContent>
