...
export function DormerFinesPage() {
  const { user } = useAuth();
  const { impositions, summary, loading } = useDormerFines(user?.id ?? null);
  ...
  const balance = summary?.remaining ?? 0;
      ...
      <div className="flex flex-col gap-2 sm:gap-3">
        <div className="flex-1">
          ...
          <p className="text-xs sm:text-sm md:text-base text-[#12372A] mt-1 sm:mt-1.5">
            View your fine charges and balances
          </p>
        </div>
      </div>
      ...
      <Card className="border border-gray-200 shadow-md bg-white gap-0">
        <CardHeader className="border-b border-gray-100 md:pb-0">
          ...
          <CardDescription className="text-xs sm:text-sm text-gray-600 mt-1">
            All fines imposed on you and their statuses
          </CardDescription>
        </CardHeader>
          ...
          {impositions.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 sm:py-16 px-3 sm:px-4">
              ...
              <p className="text-xs sm:text-sm md:text-base text-gray-600 text-center max-w-md px-4">
                You don't have any fine records yet. They will appear here once
                fines are generated.
              </p>
            </div>
          ) : (
            <div className="p-3 sm:p-4 md:p-6 space-y-6">
              <FinesTable impositions={impositions} />
...
function FinesTable({
  ...
  impositions: FineImpositionWithCategory[];
}) {
  return (
    <div className="overflow-x-auto rounded-lg border border-gray-200">
      <Table>
        ...
        <TableBody>
          {impositions.map((f) => {
            const balance = Math.max(0, f.amount - f.amount_paid);
            return (
              <TableRow
                key={f.id}
